...
//! 
//! [`Bitset(z)`](Bitset) is a thin-as-possible abstraction that behaves exactly like a `Hashset`, but stores only a *single* number `z`. When written in binary, the bits of `z` represent bitflags for each of the integers in `1..=N` – `1` if the integer is present in the set, `0` if not.
//! 
